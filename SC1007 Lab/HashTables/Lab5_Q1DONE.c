...
{
    for(int i=0; i<Q3Hash.hSize; i++)
    {
        ListNode *temp = Q3Hash.Table[i].head;
...
{
    int hashedkey = Hash(key, Q3HashPtr->hSize);
    
    ListNode *new = malloc(sizeof(ListNode));
    new->key = key;
    new->next  = NULL;

    ListNode * node = Q3HashPtr->Table[hashedkey].head;

    new->next = node;
    Q3HashPtr->Table[hashedkey].head = new;
    return 1;
}
