...
void adjM2adjL(Graph *g)
{
    ...
    list = (ListNode**) malloc(g->V*sizeof(ListNode*)); // allocate memory for list of all vertices
        ...
        for(j=0; j<g->V; j++)
        {
            if( g->adj.matrix[i][j] == 1)
            {
                if( list[i] == NULL)
                {
                    ...
                    list[i]->vertex = j+1; // j+1 because if 1 and 2 are connected means [0][1]==1
                    list[i]->next = NULL;
                    temp = list[i];
                }
                else
...
void calDegreeV(Graph g, int *degreeV)
{
    ...
    {
        printf("dont use adjacency matrix !");
        return;
    }
