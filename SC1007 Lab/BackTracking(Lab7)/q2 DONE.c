...
int nQueens(int** board, int BSize, int col)
{
     ...
     if(col >= BSize)
     {
        ...
        return 1;
         ...
         for( int i = 0; i<BSize; i++)  // i refers to the column
        {
...
int isSafe(int** board,int BSize, int row, int col)
{
    ...
    // Horicatal check        -> This checks if there are any queens on this row
    for (i = 0; i < col; i++)
        if (board[row][i])
            return 0;

    // Upper left diagonal check  -> checks if there are any queen on one type of diagonal
    for (i = row, j = col; i >= 0 && j >= 0; i--, j--)
        if (board[i][j])
            return 0;

    // Lower left diagonal check   -> check the other type of diagonal
